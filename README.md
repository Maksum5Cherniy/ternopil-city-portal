...
- Публічні сторінки, пошук за розділами й інтерактивна карта працюють без бази даних.
- Новини та події зі стартових даних показуються як архів. Приклади оголошень не видаються за активні продажі.
- Реєстрація, оголошення, відгуки, модерація та адмінка потребують `DATABASE_URL`. Для підтвердження адрес і відновлення паролів потрібен також `RESEND_API_KEY` із налаштованим доменом відправника.
- Запити, що змінюють дані, перевіряють походження; спроби входу й відновлення обмежуються через Postgres. Посилання підтвердження та відновлення не виводяться в серверні журнали. Обране зберігається в браузері окремо для кожного профілю.
