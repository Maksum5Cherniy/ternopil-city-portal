...
4. Виконати `npx vercel env pull .env.local --yes --scope de-te`.
5. Запустити `npm run seed` локально або окремим одноразовим production-скриптом.
...
Роль `admin` задається в масиві `users.roles`. Змінна `ADMIN_EMAILS` автоматично додає роль `admin` для відповідних email під час реєстрації або входу.
